...
final class BroadcastExclusionPingTests: XCTestCase {
    func testSuccessfulPingToBroadcastIsIgnored() async {
        ...
        let createdDevice = await MainActor.run { store.devices.first }
        XCTAssertEqual(finalCount, 0, "Broadcast address should be ignored and not create a device. Created device: \(String(describing: createdDevice))")
    }
}
